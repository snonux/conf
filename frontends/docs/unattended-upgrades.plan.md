...
2. **`base` mode**:
   ...
   - Re-runs syspatch once if the first run only installed the tool self-update (001_syspatch exits 2 with errata pending). Diff empty → exit silently. Diff non-empty → log + mail the applied patch list; if the kernel was patched (KARL-safe check: `what(1)` build version of `/bsd` vs the booted version in `dmesg.boot` line 1 — the original `/bsd`-newer-than-`dmesg.boot` mtime idea is unusable because KARL re-links `/bsd` at every boot), log that a reboot is queued — there is NO needs-reboot flag; the `reboot` window decides purely on that same version compare; non-kernel base patches (e.g. libc) instead restart the curated service list too, since daemons keep old code until restarted.
3. **`pkgs` mode**:
   - Pre-check: ≥ 256 MB free on `/`, `/usr`, `/var` each, else mail + abort (1 GB was unreachable on the frontends' small root).
   - Probe the custom fleet repo first (its relayd front serves an HTTP-200 "Server turned off" page when the k3s backend is down): up → export `PKG_PATH=installpath:<custom fleet repo>`; down → log a WARNING, run `pkg_add -Iu` official-only, and treat the resulting rc=1 ("Couldn't find updates" for custom stems) as an expected warning — the unattended run must not fail when the cluster is down. Nothing to do → silent. Otherwise log + mail the update list.
...
4. **`reboot` mode**: if the KARL-safe kernel version compare says the on-disk kernel differs from the booted one → log, `reboot` (rc scripts bring up relayd/httpd/nsd/etc.; brief downtime inside the host's own window, hosts staggered). No flag state exists; manually applied kernel patches are picked up the same way. dns-failover flips zones during a reboot by design (its 3-consecutive-failure threshold tolerates mere restart blips) and flips back afterwards.
